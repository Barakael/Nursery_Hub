...
const AdminDashboard = () => {
  ...
  const stats = [
    ...
    { icon: CreditCard, label: "Collected", value: `TSh ${collected.toLocaleString()}`, color: "bg-success/15 text-success" },
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="rounded-2xl bg-card p-5 shadow-card">
        ...
        <div className="mb-3 flex justify-between text-sm">
          <span className="text-muted-foreground">Target: TSh {total.toLocaleString()}</span>
          <span className="font-bold text-card-foreground">TSh {collected.toLocaleString()} ({pct}%)</span>
        </div>
              ...
              return (
                <div key={s.id} className="flex items-center justify-between rounded-xl bg-accent/50 px-4 py-3">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-bold text-card-foreground">TSh {s.total_paid.toLocaleString()}</p>
